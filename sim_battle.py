...
def simulate_battle(army_one, army_two, terrain):
    ...
    # BATTLE
    print(f'Rounds: {rounds} - Terrain: {terrain}')
    day = 0
    while day < rounds:
        day += 1
        if day < (rounds * 0.25):
        ...
        if day % 2 == 0:
            ...
            attack_stats = army_one_attack / 20
            defence_army = army_two
            defence_stats = army_two_defence / 20
        else:
            attack_army = army_two
            attack_stats = army_two_attack / 20
            defence_army = army_one
            defence_stats = army_one_defence / 20
    ...
    return {"army_one_morale": round(army_one_morale, 0), "army_two_morale": round(army_two_morale, 0),
            "army_one_casualties": round(army_one_casualties, 0), "army_two_casualties": round(army_two_casualties, 0), "rounds": rounds}
